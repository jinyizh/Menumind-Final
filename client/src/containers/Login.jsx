...
const Login = () => {
  ...
  const signUpWithEmailPass = async () => {
    if (userEmail === '' || password === '' || confirm_password === '') {
        dispatch(alertInfo('Should not be empty'))
    }
    else {
  ...
  const signInWithEmailPass = async () => {
    if (userEmail !== "" && password !== "") {
        ...
        dispatch(alertInfo('Password does not match'))
    }
  }
    ...
    <div className='w-screen h-screen relative overflow-hidden flex'>
        ...
        <div className='flex flex-col items-center bg-lightOverlay w-[80%] md:w-508 h-full z-10 backdrop-blur-md p-4 px-4 py-12 gap-6'>
            ...
            <div className='flex items-center justify-start gap-4 w-full'>
                ...
                <p className='text-headingColor font-semibold text-2xl'>Menumind</p>
            </div>
            {/* welcome text */}
            <p className='text-3xl font-semibold text-headingColor'>Welcome Back!</p>
            <p className='text-xl text-textColor'>{isSignUp? "Sign up": "Sign in"} with following</p>
            {/* input section */}
